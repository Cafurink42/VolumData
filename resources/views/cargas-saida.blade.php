<x-app-layout>
   <x-slot name="header">
    ...
    <form action = "{{route('cargas.saida.store')}}" method = "POST" >
        ...
        <x-text-input id="peso_saida" class="block mt-1 w-[30%]"
                            type="number"
                            name="peso_saida"
                            required autocomplete="peso_saida" />

        <x-input-label for="data_saida" :value="__('Data de Saída')" />
        <x-text-input id="data_saida" class="block mt-1 w-[30%]"
                            type="date"
                            name="data_saida"
                            required autocomplete="data_saida" />
       ...
       <x-text-input id="empresa_destino" class="block mt-1 w-[30%]"
        ...
        <div class = "flex justify-center items-center mt-[-10%]">
            <x-primary-button type="submit" class="mt-6 ml-4 bg-red-500 hover:bg-red-700 text-white;  font-bold py-2 px-4 r">
                {{ __('Registrar Saida')}}
            </x-primary-button>
        </div>
    </form>
